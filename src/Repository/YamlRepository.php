...
final readonly class YamlRepository implements ResolverRepositoryInterface
{
     ...
     * @throws \RuntimeException If a file cannot be read or parsed, or if symfony/yaml is not installed
     */
    public function __construct(
        string|array $paths,
        private ?string $basePath = null,
    ) {
        throw_unless(\class_exists(Yaml::class), \RuntimeException::class, 'YamlRepository requires symfony/yaml package. Install it with: composer require symfony/yaml');
    ...
    private function loadResolvers(string|array $paths): array
    {
        ...
        foreach ($paths as $path) {
            ...
            throw_unless(\file_exists($fullPath), \RuntimeException::class, 'YAML file not found: ' . $fullPath);

            throw_unless(\is_readable($fullPath), \RuntimeException::class, 'YAML file not readable: ' . $fullPath);

            try {
                $data = Yaml::parseFile($fullPath);
            } catch (\Exception $e) {
                throw new \RuntimeException(sprintf('Invalid YAML in file %s: %s', $fullPath, $e->getMessage()), 0, $e);
            }

            throw_unless(\is_array($data), \RuntimeException::class, 'YAML file must contain a mapping/array: ' . $fullPath);
