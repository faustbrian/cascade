...
describe('YamlRepository', function (): void {
    ...
    describe('constructor', function (): void {
        ...
        test('throws YamlFileNotFoundException when file does not exist', function (): void {
            // Arrange
            $nonExistentPath = $this->tempDir.'/nonexistent.yaml';

            // Act & Assert
            expect(fn (): YamlRepository => new YamlRepository($nonExistentPath))
                ->toThrow(YamlFileNotFoundException::class, 'YAML file not found: '.$nonExistentPath);
        });

        test('throws YamlFileNotReadableException when file is not readable', function (): void {
            ...
            try {
                ...
                expect(fn (): YamlRepository => new YamlRepository($filePath))
                    ->toThrow(YamlFileNotReadableException::class, 'YAML file not readable: '.$filePath);
            } finally {
        ...
        test('throws InvalidYamlFileException when YAML is invalid', function (): void {
            // Arrange
            ...
            // Act & Assert
            expect(fn (): YamlRepository => new YamlRepository($filePath))
                ->toThrow(InvalidYamlFileException::class, 'Invalid YAML in file');
        });

        test('throws YamlFileMustContainArrayException when YAML file does not contain array', function (): void {
            // Arrange
            ...
            // Act & Assert
            expect(fn (): YamlRepository => new YamlRepository($filePath))
                ->toThrow(YamlFileMustContainArrayException::class, 'YAML file must contain a mapping/array: '.$filePath);
        });

        test('symfony/yaml package availability is verified', function (): void {
            ...
            // The actual check in constructor would throw YamlPackageRequiredException
            // if symfony/yaml wasn't installed
        });
    });
    ...
    describe('edge cases', function (): void {
        test('throws YamlFileMustContainArrayException for empty YAML file', function (): void {
            // Arrange
            $filePath = $this->tempDir.'/empty.yaml';
            ...
            expect(fn (): YamlRepository => new YamlRepository($filePath))
                ->toThrow(YamlFileMustContainArrayException::class, 'YAML file must contain a mapping/array');
        });

        test('throws YamlFileMustContainArrayException for YAML file with only comments', function (): void {
            // Arrange
        ...
        test('handles very large YAML files', function (): void {
            ...
            // Act & Assert
            expect($repository->all())->toHaveCount(1_000);
            expect($repository->has('resolver-1'))->toBeTrue();
