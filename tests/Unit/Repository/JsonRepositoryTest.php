...
describe('JsonRepository', function (): void {
    ...
    describe('error handling - file not readable', function (): void {
        test('throws RuntimeException when file is not readable', function (): void {
            // Arrange
            $jsonFile = $this->tempDir.'/unreadable.json';
            file_put_contents($jsonFile, '{}');
            chmod($jsonFile, 0o000);

            // Act & Assert
            try {
                new JsonRepository($jsonFile);
            } catch (RuntimeException $runtimeException) {
                expect($runtimeException->getMessage())->toContain('JSON file not readable');
            } finally {
                // Clean up: restore permissions
                chmod($jsonFile, 0o644);
            }
        })->skip(fn (): bool
            // Skip on Windows or if running as root (can read despite permissions)
            => \PHP_OS_FAMILY === 'Windows' || posix_getuid() === 0);
    });
