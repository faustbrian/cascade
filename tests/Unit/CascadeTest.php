...
describe('Cascade facade', function (): void {
    ...
    describe('source conductor', function (): void {
        test('resolves value from first matching source', function (): void {
            $value = $this->cascade->from(new ArraySource('primary', ['key' => 'primary-value']))
        ...
        test('falls back to next source if first source returns null', function (): void {
            $value = $this->cascade->from(new NullSource('null-source'))
                ->fallbackTo(new ArraySource('fallback', ['key' => 'fallback-value']))
                ->get('key');
        ...
        test('resolves with context', function (): void {
            $value = $this->cascade->from(new CallbackSource(
                'context-aware',
        ...
        test('skips sources that do not support the key', function (): void {
            $value = $this->cascade->from(new CallbackSource(
                ...
                ->fallbackTo(new ArraySource('fallback', ['unsupported-key' => 'fallback-value']))
                ->get('unsupported-key');
        ...
        test('resolves with full result metadata', function (): void {
            $result = $this->cascade->from(new ArraySource('primary', ['key' => 'value']))
                ->resolve('key');
        ...
        test('getOrFail returns value when found', function (): void {
            $value = $this->cascade->from(new ArraySource('primary', ['key' => 'value']))
                ->getOrFail('key');
        ...
        test('getOrFail throws exception when not found', function (): void {
            expect(fn() => $this->cascade->from(new NullSource('null-source'))->getOrFail('key'))
                ->toThrow(ResolutionFailedException::class);
        });

        test('resolves multiple keys with getMany', function (): void {
            $results = $this->cascade->from(new ArraySource('source', [
                'key1' => 'value1',
                'key2' => 'value2',
                'key3' => 'value3',
        ...
        test('can be registered as named resolver', function (): void {
            $this->cascade->from(new ArraySource('source', ['key' => 'value']))
                ->as('my-resolver');
        ...
        test('transformers apply to resolved values', function (): void {
            $value = $this->cascade->from(new ArraySource('source', ['key' => 'value']))
        ...
        test('auto-increments priority for fallback sources', function (): void {
            $value = $this->cascade->from(new ArraySource('first', ['key' => 'first']))
                ->fallbackTo(new ArraySource('second', ['key' => 'second']))
                ->fallbackTo(new ArraySource('third', ['key' => 'third']))
                ->get('key');
    ...
    describe('resolution conductor', function (): void {
        ...
        test('binds context from model with getKey', function (): void {
            ...
            $this->cascade->defineResolver('model-resolver')
                ->source('source', new CallbackSource(
                    'source',
                    fn(string $key, array $ctx): mixed => $ctx[strtolower(class_basename($model::class)) . '_id'] ?? null,
                ));
    ...
    describe('events', function (): void {
        test('emits SourceQueried event for each attempted source', function (): void {
            ...
            $this->cascade->from(new NullSource('null-1'))
                ->fallbackTo(new ArraySource('array', ['key' => 'value']))
                ->resolve('key');
        ...
        test('emits ResolutionFailed event when not found', function (): void {
            ...
            $this->cascade->from(new NullSource('null-source'))
                ->resolve('key');
    ...
    describe('priority ordering', function (): void {
        test('queries sources in priority order', function (): void {
            $value = $this->cascade->from(new ArraySource('low-priority', ['key' => 'low']), priority: 10)
                ->addSource(new ArraySource('high-priority', ['key' => 'high']), priority: 1)
                ->addSource(new ArraySource('medium-priority', ['key' => 'medium']), priority: 5)
                ->get('key');
        ...
        test('default priority is 0', function (): void {
            $value = $this->cascade->from(new ArraySource('default', ['key' => 'default']))
                ->addSource(new ArraySource('low-priority', ['key' => 'low']), priority: 10)
                ->get('key');
